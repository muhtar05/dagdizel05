...
class Image
{
    ...
    public function waterMark($waterMarkImgUrl, $realImg)
    {
        $arrDetails = getimagesize($realImg);
        $watermark = imagecreatefrompng($waterMarkImgUrl);
        imagealphablending($watermark, true);
        /**
         * Изменение размера подписи
         */
        $percent = ($arrDetails[0] <= 300 ? 20 : 10);
        $src_w = imagesx($watermark);
        $src_h = imagesy($watermark);
        $dst_w = floor($arrDetails[0] * $percent / 100);
        $dst_h = (int)(($src_h * $dst_w) / $src_w);
        //echo '$src_w = '.$src_w.' $src_h = '.$src_h.' $dst_w = '.$dst_w.' $dst_h = '.$dst_h.'<br />';
        $image2 = imagecreatetruecolor($dst_w, $dst_h);
        imagealphablending($image2, false);
        imagesavealpha($image2, true);
        imagecopyresampled($image2, $watermark, 0, 0, 0, 0, $dst_w, $dst_h, $src_w, $src_h);
        $watermark = $image2;
        /**
         * КОНЕЦ
         */
...
//        print_r($arrDetails);
//        echo $watermark_width."x".$watermark_height;
//        header('content-type: ' . $arrDetails['mime']);
        switch ($arrDetails['mime']) {
            case 'image/jpeg':
                $image = imagecreatefromjpeg($realImg);
                $dest_x = $arrDetails[0] - $watermark_width;
                $dest_y = $arrDetails[1] - $watermark_height;
                //imagecolortransparent($watermark,imagecolorat($watermark,0,0));
                imagecopyresampled($image, $watermark, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height, $watermark_width, $watermark_height);
                //imagecopymerge($image, $watermark, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height, 100);

                imagejpeg($image, $realImg, 100);
                //imagejpeg($image);
                break;
            case 'image/png':
                $image = imagecreatefrompng($realImg);
                imagealphablending($image, true);
                imagesavealpha($image, true);
                $dest_x = floor($arrDetails[0] / 2) - floor($watermark_width / 2);
                $dest_y = floor($arrDetails[1] / 2) - floor($watermark_height / 2);
//                            imagecopyresampled($image, $watermark, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height, $watermark_width, $watermark_height);
                imagecopyresampled($image, $watermark, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height, $watermark_width, $watermark_height);
                imagepng($image, $realImg, 9);
                //imagepng($image);
                break;
            case 'image/gif':
                $image = imagecreatefromgif($realImg);
                $dest_x = floor($arrDetails[0] / 2) - floor($watermark_width / 2);
                $dest_y = floor($arrDetails[1] / 2) - floor($watermark_height / 2);
                imagecopyresampled($image, $watermark, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height, $watermark_width, $watermark_height);
                imagegif($image, $realImg);
                break;
        }
        imagedestroy($image);
