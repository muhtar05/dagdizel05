...
class UserController extends Controller
{
	...
	public function accessRules()
	{
		return array(
            ...
            array('allow', // allow admin user to perform 'admin' and 'delete' actions
                'actions'=>array('editUser'),
                'users'=>User::model()->getUsersAccessArrayByRole(User::USER),
            ),

            array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('create','update','admin','delete','editUser'),
    ...
    public function actionRegistration()
    {
        ...
        if (Yii::app()->request->isPostRequest && !empty($_POST['RegistrationForm']))
        {
            ...
            {
                $user = new User;
                $user->createAccount($form->username,$form->email,$form->password);
    ...
    public function actionEditUser($id){
        $this->layout = '//layouts/column1';
        $sessionUserId = Yii::app()->user->id;
        $userAuth = User::model()->findByPk($sessionUserId);
        $model = $this->loadModel($id);
        if ($userAuth->roleId != User::ADMIN) {
